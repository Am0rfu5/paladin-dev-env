...
ones). Reference: @/workspace/.claude/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / /workspace/.claude.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  "/workspace/.claude/gsd-core/bin/lib/edge-probe.cjs" \
  "/workspace/.claude/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
requirement list). Reference: @/workspace/.claude/gsd-core/references/prohibition-probe.md — the
...
Use the SPEC.md template from @/workspace/.claude/gsd-core/templates/spec.md.
