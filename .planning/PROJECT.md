...
| [Grove routing model from configuration](.planning/decisions/0013-grove-routing-model.md) (ADR-0013) | `GroveConfig.routing_model: Option<String>` replaces the hardcoded `"gpt-4"` literal; when absent under `RoutingStrategy::LlmRouting`, routing hard-errors with `BattalionError::RoutingError` and no fallback of any kind, a deliberate one-way runtime break the human user approved as `proceed-as-locked`. | ✓ Good — applied in Phase 6 (plan 06-01: field, builder setter, and guard shipped and tested) |
