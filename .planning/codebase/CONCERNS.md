...
**Amended by Phase 9 (plan 09-07), dated 2026-08-08, citing `.planning/decisions/0024-rustsec-exception-governance.md` (ADR-0024) and `SECURITY-EXCEPTIONS.md`:** the "10 unmaintained crates" count above and the list itself are stale. Re-read directly against the pre-Phase-9 tree (`ADR-0024`'s verbatim `Cargo.lock` liveness transcript, run this session), `deny.toml` held **nine** unmaintained entries, not ten — `RUSTSEC-2025-0121` (`gcc`) had already been removed from `deny.toml`, from `.cargo/audit.toml`, and from `Cargo.lock` before this phase started, with no record anywhere of when or by whom. Phase 8's clap v4 migration (`ADR-0023`) then removed `structopt` from the manifest entirely, taking three further `deny.toml` entries dead with it: `RUSTSEC-2022-0104` (`structopt`), `RUSTSEC-2021-0139` (`ansi_term`) and `RUSTSEC-2024-0375` (`atty`) all justified themselves "via structopt"/"via clap 2.x/structopt" and returned zero hits in `Cargo.lock` this session, alongside `RUSTSEC-2024-0370` (`proc-macro-error`). Plan 09-06 (commit `6513cb7`) **deleted** those four dead entries — attaching an owner and a review date to a suppression that suppresses nothing would be governance theatre, per ADR-0024 decision. **The live unmaintained set today is five**: `dotenv`, `fxhash`, `number_prefix`, `rustls-pemfile`, `paste`. The migration plan's items 1 and 4 (replace `structopt`, await upstream for `ansi_term`/`atty`) are moot — their target crates no longer ship. `SECURITY-EXCEPTIONS.md` (repo root) is now the authoritative governance register for every live suppression — owner, review date, affected scope and compensating control for each; the original list and migration plan above are retained verbatim, per the amend-at-source convention.
...
**Amended by Phase 9 (plan 09-07), dated 2026-08-08, citing `.planning/decisions/0024-rustsec-exception-governance.md` (ADR-0024) and `SECURITY-EXCEPTIONS.md`:** the `deny.toml:141–147` citation above is stale — plan 09-03's unrelated nine-line deletion in `Dockerfile.chef` did not move this file, but the citation was already off before this phase (a pre-existing drift this map never caught). Re-derived this session, the three 2026 advisories currently live at `deny.toml:120-122`. All three are **ratified**, not merely documented-without-a-fix: ADR-0024 decision 3 records each with a concrete compensating control naming the actual reachable input path (Paladin does not feed attacker-controlled PDF input to `lopdf` in a default build; the `s3` feature's XML input is response bodies from a project-configured S3-compatible endpoint, not third-party documents), owner `DF3NDR`, and a `2026-12-31` review date in `SECURITY-EXCEPTIONS.md`. The Mitigation list's first two items are satisfied by that register rather than by a `README.md`/release-notes note; the third (a CI check) is superseded by `scripts/check-advisory-register.sh`, which fails CI if any suppressed advisory's crate leaves `Cargo.lock` or any register row goes ungoverned. Original text retained verbatim above.
