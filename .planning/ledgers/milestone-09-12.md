...
2. **`.github/workflows/ci.yml` has 15 job ids, not the 14 `intel/code-verification.md:539-540`
   ...
   **Updated (dated 2026-08-13, Phase 15, plan 15-10):** the count has grown to **19**, re-counted
   at execution time. `python3 -c "import yaml; print(len(yaml.safe_load(open('.github/workflows/ci.yml'))['jobs']))"`
   → 19. Phase 15 added four jobs: `actionlint` (plan 15-02), `coverage`, `cli-tests`, `bench-check`
   (all plan 15-01). Full job id list, in file order: `lint`, `actionlint`, `security-audit`,
   `cargo-deny`, `osv-scanner`, `api-surface`, `test`, `examples`, `crate-isolation`,
   `integration-tests`, `coverage`, `cli-tests`, `bench-check`, `docker`, `kubernetes-smoke`,
   `e2e-tests`, `benchmark`, `benchmark-regression-signal`, `publish-dry-run`. The fifteen-job
   figure above is retained, marked superseded, per D-00d.
...
| Requirement | Verdict |
|---|---|
| REQ-ci-cli-snapshot-job | **Verified open → PIPE-01** — re-confirmed this session: `grep -n "cli-tests" .github/workflows/ci.yml` → zero matches; the corrected 15-job `ci.yml` list in this file's head note carries no such job. The 43 CLI snapshot tests are real and re-counted fresh: `tests/cli/mod.rs` registers `error_output_test` (15), `help_output_test` (12), `progress_output_test` (8), `table_output_test` (8) — `grep -c '#\[test\]'` on each, re-run this session, sums to **43** — behind the `"cli"` `[[test]]` target's `required-features = ["cli"]` (`Cargo.toml:211-213`). Confirmed they never run: the `test` job scopes to `cargo test --workspace --lib --bins` (`ci.yml:238`, no `[[test]]` integration targets at all); `integration-tests.yml:111`'s `cargo test --workspace --features integration-tests` does not pass the `cli` feature, so Cargo silently skips the target — verified by reading both invocations, not inferred. **Closed (dated 2026-08-13, Phase 15, plan 15-10):** this row's own "sums to 43" arithmetic is corrected in place, not just the requirement text — it enumerated four of the seven `tests/cli/*.rs` files carrying `#[test]` functions. Re-counted at execution time: `ls tests/cli/snapshots \| wc -l` → **86** snapshot files; `grep -c '#\[test\]' tests/cli/*.rs` summed across all fifteen files → **97** test functions, non-zero in seven (`error_output_test.rs` 15, `environment_tests.rs` 44, `progress_output_test.rs` 8, `integration_tests.rs` 3, `error_handling_test.rs` 7, `help_output_test.rs` 12, `table_output_test.rs` 8). Shipped: `.github/workflows/ci.yml` job id `cli-tests` (name "CLI Snapshot Tests"), plan 15-01 commit `16b34f8`; CI run `31727496744` job "CLI Snapshot Tests" → success, commit `e9e3267f9ae6d8483be3ee52c04ffe6a763cbb37` (per `15-04-SUMMARY.md`) |
| REQ-ci-bench-check-job | **Verified open → PIPE-01** — re-confirmed this session: `grep -n "bench-check" .github/workflows/ci.yml` → zero matches. The inversion still holds, with a corrected line number (D-08): `benchmark-regression-signal`, which this Epic's own non-goals place out of scope, ships at `ci.yml:812` (not the stale `:531` — the head note's re-measured 15-job list moved it), while the `cargo bench --no-run` compile-check prerequisite this row targets does not exist anywhere in `ci.yml`. **Closed (dated 2026-08-13, Phase 15, plan 15-10):** shipped as `.github/workflows/ci.yml` job id `bench-check` (`cargo bench --workspace --no-run`, compile-only), plan 15-01 commit `16b34f8`; CI run `31727496744` job "Benchmark Compile Check" → success, commit `e9e3267f9ae6d8483be3ee52c04ffe6a763cbb37` (per `15-04-SUMMARY.md`). The pre-existing `benchmark`/`benchmark-regression-signal` jobs are untouched |
| REQ-ci-combined-coverage-job | **Verified open → PIPE-02** — re-confirmed this session: `grep -n "^  coverage:" .github/workflows/ci.yml` and `grep -n "llvm-cov\|codecov" .github/workflows/ci.yml` both return zero. **But coverage tooling is partially built**: `integration-tests.yml:117-123` runs `cargo llvm-cov --features integration-tests --lcov --output-path integration-lcov.info` (`:118`) and uploads via `codecov/codecov-action@v3` (`:123`) — both lines re-read this session — the integration-only path this requirement supersedes rather than fills a void. Epic 25's own **Open Question 3** — remove the `integration-tests.yml` step once `ci.yml` coverage lands, or keep both — is unanswered. Owner **Phase 15 / PIPE-02**. **Closed (dated 2026-08-13, Phase 15, plan 15-10):** `.github/workflows/ci.yml` job id `coverage` runs `cargo llvm-cov --workspace --features integration-tests --lcov --output-path lcov.info --fail-under-lines 82 -- --test-threads=1` (plan 15-01 commit `e54a94f`, armed by plan 15-03 commit `3454749`); the `integration-tests.yml` duplicate step was **removed** (Epic 25's own OQ-3, answered by removal — plan 15-02 commit `f9b5ad2`). CI run `31727496744` job "Coverage" → success, `Lines: 39233/47618 = 82.39%`, commit `e9e3267f9ae6d8483be3ee52c04ffe6a763cbb37` |
| REQ-codecov-config-thresholds | **Verified open → PIPE-02** — re-confirmed this session: neither `.codecov.yml` nor `codecov.yml` exists at repo root (`ls` fails for both). **Both sides of the threshold variant recorded, neither resolved (variant group 30):** the parent PRD `prd-deferred-qa-completion.md` FR-25.3 (`:113-116`) sets a flat **78% hard gate** — "PRs dropping below this threshold must fail"; Epic 25's own `prd-cicd-pipeline-enhancement.md` FR-25.6 (`:192-238`) specifies a phased **70% → 74% → 78%** ramp across three sprints instead, explicitly to avoid blocking PRs during Epics 28-29's coverage build-out. The parent PRD's own **OQ-3** (`:492`) — hard-fail vs. soft-warning during ramp-up — is recorded **Open** in the source document itself, not merely inferred. `DEFERRED_COVERAGE.md`'s measured baseline ("Overall coverage (76-77%)") is the number either side would ramp from. Owner **Phase 15 / PIPE-02** picks; this row picks neither. **Closed (dated 2026-08-13, Phase 15, plan 15-10):** neither side. `.codecov.yml` lands report-only (`informational: true` on both `project` and `patch` status blocks, plan 15-02 commit `7d74caa`) and never gates; the binding threshold is `.planning/decisions/0006-coverage-gate.md`'s `## Phase 15 amendment (2026-08-13)` — one number, 82%, derived fresh from the 82.39% measurement, neither inherited position. Both the 78% hard gate and the 70→74→78 ramp are recorded rejected in ADR-0006's `## Considered Options` |
| REQ-makefile-coverage-targets | **Verified open → PIPE-03** — re-confirmed this session: `grep -nE '^(coverage\|coverage-html\|test-cli\|bench-check):' Makefile` → zero matches for all four targets; `grep -n 'llvm-cov' Makefile` → zero matches, the `Makefile` has no coverage-tooling reference at all. Owner **Phase 15 / PIPE-03**. **Closed (dated 2026-08-13, Phase 15, plan 15-10):** all five targets shipped — `coverage`, `coverage-html`, `test-cli`, `bench-check` (plan 15-01 commit `a6653b5`) plus `ci-full: ci-test coverage` (new). `--fail-under-lines 82` armed on `coverage` (plan 15-03 commit `3454749`), byte-identical to the CI job's flag set (verified by `diff` of the extracted `llvm-cov ... test-threads=1` substring) |
| REQ-modernize-github-actions | **Partially open → PIPE-04** — re-confirmed this session: the dangling `on: schedule` block is **gone** (`ci.yml` has exactly one `on:` block, at line 3, `push`/`pull_request`/`workflow_dispatch` only — no `schedule:`/`cron:` anywhere in the file), the **only** Epic 25 item found closed. Eight deprecated references remain, re-grepped this session with **corrected line numbers** (D-08 — `ci.yml`'s shape changed since run 5's stale `:147,317,507` citation): `actions-rs/toolchain@v1` at `ci.yml:163,408,788` and `integration-tests.yml:71` (4); `actions/cache@v3` at `integration-tests.yml:78,84,90` (3); `codecov/codecov-action@v3` at `integration-tests.yml:123` (1) — 4+3+1 = 8, re-confirmed. Owner **Phase 15 / PIPE-04**. **Closed (dated 2026-08-13, Phase 15, plan 15-10):** **seven upgraded, one deleted** (not eight upgraded) — `codecov/codecov-action@v3` was removed rather than upgraded (its replacement is the `coverage` job's `codecov/codecov-action@v5` upload) — `grep -rn 'actions-rs/\|cache@v3\|codecov-action@v3' .github/workflows/` → zero matches. Plan 15-02, commits `f9b5ad2` (upgrades and deletion) and `801a300` (the new standing `actionlint` job, all six workflow files, zero findings) |
| REQ-contributing-coverage-docs | **Verified open, and the target document changed shape → PIPE-05** — re-confirmed this session: no top-level `CONTRIBUTING.md` exists in the tree (Milestone 11's mdbook migration relocated contributor docs under `docs/src/contributing/`). FR-25.8's content is **partially present but split and incomplete**: `docs/src/contributing/testing-guide.md:394-431` already has a `## Test Coverage` section with install (`cargo install cargo-llvm-cov`), local HTML-report generation, and CI-format lcov generation commands — but no explicit threshold-expectations-vs-CI comparison or troubleshooting subsection, and no `cargo tarpaulin`-to-`cargo-llvm-cov` migration note (FR-25.8 item 24) anywhere in the file (re-grepped this session). Owner **Phase 15 / PIPE-05** inherits a partial-content gap, not a from-scratch write. **Closed (dated 2026-08-13, Phase 15, plan 15-10):** `docs/src/contributing/testing-guide.md`'s `## Test Coverage` section rewritten end-to-end (prerequisites, local generation, the `--features integration-tests` scope rationale, the 82% threshold policy, reading output, Codecov's non-gating behaviour, troubleshooting), plan 15-04 commit `b6baf96`; the `cargo tarpaulin` migration note landed in `.planning/codebase/TESTING.md` instead of `CONTRIBUTING.md` (the only tarpaulin references in the tree were there, not in contributor docs), commit `b7c74b2`. The local-reproduction half of "done when" is deferred, tracked at `.planning/todos/pending/2026-08-13-verify-local-coverage-reproduction.md`, per the user's recorded "Accept, track local check" decision |
...
| Requirement | Verdict |
|---|---|
| REQ-mock-infrastructure | **Verified open in the specified shape, with one correction → DEFER-01.** Re-confirmed this session: `test -d tests/common` fails — no such directory; mocks live at `tests/helpers/{mock_llm_adapter,mock_arsenal_adapter,mock_paladin_port}.rs` (`tests/helpers/mod.rs` re-checked), a different location and a disjoint set from the five named. Four of the five named mocks are genuinely absent — `grep -rn 'MockUserRepository\|MockNotificationService\|MockEventSource\|MockTriggerExecutor' tests/ src/ crates/` returns zero. **Correction:** the fifth, `MockLogPort`, **does exist** — `grep -rn 'struct MockLogPort'` finds it at `src/application/services/log_orchestrator/mod.rs:372` and, separately, at `src/config/user_config.rs:74` — but as a module-local `#[cfg(test)]` double at each of two call sites, not shared reusable infrastructure under `tests/`. Recorded as a partial correction to the run-5 premise rather than a full miss: 4/5 absent, 1/5 present-but-unshared. **The shared prerequisite for both coverage epics**, ~6-10 of the 35-45 estimated hours. Owner **Phase 15 / DEFER-01**. **Closed (dated 2026-08-13, Phase 15, plan 15-10):** `src/test_support/` shipped (`#[cfg(test)]`-gated at the `src/lib.rs` declaration), the `src/`-side twin `tests/helpers/` structurally could not be — plan 15-05, commits `b89b263`/`a03b043`. Demand-driven, not the five-name set built speculatively: `FailingChannelHandler` (replaces `MockNotificationService`), `event_factory` (replaces `MockEventSource`); `MockUserRepository`/`MockTriggerExecutor` judged unnecessary (real `sqlite::memory:` / real trigger store gave stronger guarantees); `MockLogPort`'s present-but-unshared finding above is **not contradicted** — a third local double (`RecordingLogPort`) was added in `user_service.rs`'s own test module (plan 15-06), still not exported to `src/test_support/`. Open Question 2 answered: hand-written, `mockall` not adopted. Full five-row verdict table in `.planning/REQUIREMENTS.md`'s DEFER-01 correction |
| REQ-user-service-test-coverage | **Open, and the collision is already resolved → DEFER-02.** Re-confirmed this session: `src/core/platform/manager/user_service.rs` still ships, 19,046 bytes / 583 lines, one of exactly four files left in `src/core/platform/manager/` (`content_service.rs`, `event_manager.rs`, `mod.rs`, `user_service.rs`). Milestone 8 `deferred-items.md` D2 planned to **split** this same file that Epic 28 plans to **test** — but **ADR-0034** (`.planning/decisions/0034-d1-d4-facade-relocation-disposition.md`) already withdrew the split: "the split is WITHDRAWN" (`:98`), narrowed to nothing by three facts including a prior phase's own finding that "no split was needed." ADR-0034 explicitly names **Phase 15 / DEFER-02** as the inheritor, sizing its ≥80%-coverage estimate and mock set against the **unsplit** file (`:240-241`) — the sequencing question this row previously left open is now a decided input, not a live collision. Owner **Phase 15 / DEFER-02**. **Closed (dated 2026-08-13, Phase 15, plan 15-10):** **94.21% line coverage (927/984 lines)**, measured via `cargo llvm-cov --workspace --lib --json --output-path /tmp/cov.json` (default features) at commit `432d514`, well above the ≥80% bar — not comparable to ADR-0006's 82.39% workspace `--features integration-tests` gate figure (different scope). 40 new tests (14 + 26) across plans 15-06 (commits `a249f68`/`298d6c3`) and 15-07 (commits `432d514`/`42ae77f`) — registration, validation, argon2 hashing, notification-failure tolerance, authentication, profile lifecycle, queries, concurrency. File grown to 1628 lines by this phase's own additions (not a further staleness). Production/behavioural findings, each named with an owner, in `.planning/REQUIREMENTS.md`'s DEFER-02 correction |
