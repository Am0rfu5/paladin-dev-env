...
/// Qdrant adapter for production-grade vector storage.
///
/// Connects to a running Qdrant instance and stores/searches [`SanctumEntry`]
...
impl QdrantSanctumAdapter {
    /// Create a new `QdrantSanctumAdapter`, connecting to the given Qdrant URL.
    ///
    ...
    ///
    /// # Errors
    /// Returns [`SanctumError::ConfigError`] if the client cannot connect, or
    /// [`SanctumError::StorageError`] if collection creation fails.
    pub async fn new(
        url: &str,
        collection_name: &str,
        vector_dimension: usize,
    ...
    /// Convert [`SanctumFilter`] to a Qdrant [`Filter`].
...
/// Convert JSON [`Value`] to Qdrant [`QdrantValue`].
...
/// Convert JSON [`Value`] to Qdrant [`MatchValue`] for filtering.
...
/// Convert Qdrant [`QdrantValue`] to JSON [`Value`].
