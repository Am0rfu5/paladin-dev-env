...
| [HTTP service host](http-service-host.md) | One long-running process, agents resident behind an API | Concurrent requests over a shared agent registry | You need request/response access to many agents | A single embedded call already suffices | **`paladin-server` (ships out of the box, `web-server` feature)**: `/v1` agent API, auth, OpenAPI docs |
