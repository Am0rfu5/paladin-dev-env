...
- [x] 6.0 Migrate and write unit tests
  - [x] 6.1 Check for existing `TokenCounterFactory` tests: run `grep -rn "TokenCounterFactory" tests/` — if no tests are found, proceed to write them in step 6.5.
  - [x] 6.2 Read `tests/unit/sanctum/memory_extraction_service_test.rs` in full. At the bottom of `crates/paladin-memory/src/services/memory_extraction_service.rs`, add a `#[cfg(test)] mod tests { use super::*; ... }` block and paste in the test content. Update any import paths that reference `paladin::` to use `paladin_core::` or `paladin_ports::` directly (FR-8.1).
  - [x] 6.3 Read `tests/unit/sanctum/rag_retrieval_service_test.rs` in full. Add an inline `#[cfg(test)] mod tests { ... }` block at the bottom of `crates/paladin-memory/src/services/rag_retrieval_service.rs` with the same import-path update treatment (FR-8.1).
  - [x] 6.4 Read `tests/unit/sanctum/qdrant_sanctum_test.rs` in full. Add an inline `#[cfg(test)] mod tests { ... }` block at the bottom of `crates/paladin-memory/src/sanctum/qdrant_adapter.rs`, wrapped in `#[cfg(all(test, feature = "qdrant"))]` (FR-8.1).
  - [x] 6.5 If no `TokenCounterFactory` tests existed (confirmed in step 6.1), add a `#[cfg(test)] mod tests { ... }` block at the bottom of `crates/paladin-memory/src/garrison/token_counter.rs` (gated on `#[cfg(all(test, feature = "content-processing"))]`) with at minimum these three tests: (a) `test_factory_creates_counter_for_known_model` — construct a factory and call `create("gpt-4")`; assert `Ok`; (b) `test_factory_returns_error_for_unknown_model` — call `create("not-a-real-model-xyz")`; assert `Err`; (c) `test_counter_counts_tokens` — create a counter for a known model and call `count_tokens("Hello, world!")`; assert the result is `Ok` and `> 0` (FR-8.1 addendum).
  - [x] 6.6 Run `cargo test -p paladin-memory --all-features` and confirm all migrated and newly written tests pass.
