...
- [ ] 1.0 User-Facing Documentation (FR-USER, FR-API)
  ...
  - [ ] 1.13 Add rustdoc comments to all public types in `src/core/platform/container/arsenal.rs` (FR-API-1)
  - [ ] 1.14 Add rustdoc comments to all public types in `src/core/platform/container/battalion/` modules (FR-API-1)
  - [ ] 1.15 Add rustdoc comments to all public traits in `src/application/ports/` (FR-API-1)
  - [ ] 1.16 Add module-level documentation (`//!`) to `src/core/mod.rs`, `src/application/mod.rs`, `src/infrastructure/mod.rs` (FR-API-3)
...
- [ ] 2.0 Technical Documentation (FR-ARCH, FR-DEPLOY, FR-OPS, FR-CONTRIB)
  - [ ] 2.1 Write `docs/architecture/overview.md` - three-layer hexagonal architecture with diagrams (FR-ARCH-1)
  - [ ] 2.2 Write `docs/architecture/hexagonal-design.md` - port/adapter pattern explanation with mapping table (FR-ARCH-2)
  - [ ] 2.3 Write `docs/architecture/domain-model.md` - DDD entities (Paladin, Battalion, Garrison, Arsenal) with relationships (FR-ARCH-1)
  - [ ] 2.4 Write `docs/architecture/design-patterns.md` - Builder, Repository, Port/Adapter, Node<T> patterns (FR-ARCH-4)
  - [ ] 2.5 Create dependency flow diagrams showing allowed/prohibited imports (FR-ARCH-3)
  - [ ] 2.6 Write `docs/deployment/docker.md` - Docker image usage, multi-architecture support, versioning (FR-DEPLOY-1)
  - [ ] 2.7 Write `docs/deployment/kubernetes.md` - K8s deployment manifests, ConfigMap, Secrets, resource limits (FR-DEPLOY-2)
  - [ ] 2.8 Write `docs/deployment/cicd.md` - GitHub Actions workflows for build, test, deploy (FR-DEPLOY-3)
  - [ ] 2.9 Write `docs/deployment/production-best-practices.md` - environment config, secret management, scaling, health checks (FR-DEPLOY-4)
  - [ ] 2.10 Write `docs/operations/logging.md` - RUST_LOG settings, structured logging, log aggregation (FR-OPS-1)
  - [ ] 2.11 Write `docs/operations/monitoring.md` - Prometheus metrics, Grafana dashboards, key metrics (FR-OPS-2)
  - [ ] 2.12 Write `docs/operations/troubleshooting.md` - common errors with causes and resolutions (FR-OPS-3)
  - [ ] 2.13 Write `docs/operations/performance-tuning.md` - Paladin config for throughput/latency, Battalion sizing, Garrison limits (FR-OPS-4)
  - [ ] 2.14 Write `docs/contributing/CONTRIBUTING.md` - dev environment setup, testing, code style, PR process (FR-CONTRIB-1)
  - [ ] 2.15 Write `docs/contributing/adapter-development.md` - LLM, Arsenal, Garrison adapter tutorials (FR-CONTRIB-2)
  - [ ] 2.16 Write `docs/contributing/testing-guide.md` - unit test, integration test, benchmark requirements (FR-CONTRIB-1)
  - [ ] 2.17 Update root `README.md` with links to documentation structure
