...
use paladin::core::base::service::node_version_service::{
    ChangeType, InMemoryNodeVersionRepository,
};
    ...
    fn fetch_content(&self, url: &str) -> Result<ContentItem, String> {
        ...
        let content = match url {
            url if url.contains("rss") => {
                let text_content =
                    TextContent::new(None, Some(format!("RSS feed content from {}", url)))
            ...
            url if url.contains("api") => {
                let text_content = TextContent::new(None, Some(format!("API data from {}", url)))
            ...
            url if url.contains("web") => {
                let text_content =
                    TextContent::new(None, Some(format!("Scraped web content from {}", url)))
...
async fn test_content_fetching_pipeline() {
    ...
    for (index, source_url) in test_sources.iter().enumerate() {
        ...
        println!(
            "   ✅ Fetched: {}",
            content_item.title().unwrap_or(&"Untitled".to_string())
        );

        // Step 2: Store content with versioning
        let content_id = content_item.uuid();
        let (stored_content, version) = content_service
            .create_content_item(
                content_item,
    ...
    for (mut content, content_id) in processed_content.into_iter() {
        ...
        // Update content
        let updated_text_content =
            TextContent::new(None, Some(processed_text)).expect("Failed to create updated content");
        ...
        let (updated_content, update_version) = content_service
            .update_content_item(
                content,
        ...
        let publish_version = content_service
            .publish_content_item(&updated_content, Some("publisher".to_string()))
        ...
        let history = content_service
            .get_version_history(content_id)
        ...
        println!(
            "   📚 Version history verified: {} versions",
...
async fn test_content_lifecycle_integration() {
    ...
    let (mut content_item, _) = content_service
        .create_content_item(
            content_item,
    ...
    for i in 1..=3 {
        ...
        content_item
            .update_content(ContentType::Text(updated_text))
            .expect("Failed to update content");

        content_item.set_tags(Some(vec![format!("update-{}", i)]));

        let (updated_content, _) = content_service
            .update_content_item(
                content_item,
    ...
    let _publish_version = content_service
    ...
    content_item
        .update_content(ContentType::Text(post_pub_text))
        .expect("Failed to update content");

    let (content_item, _) = content_service
        .update_content_item(
            content_item,
            Some("author@example.com".to_string()),
    ...
    let _archive_version = content_service
        .archive_content_item(&content_item, Some("admin@example.com".to_string()))
    ...
    let actual_changes: Vec<ChangeType> = history
        .versions
        .iter()
        .map(|v| v.metadata.change_type.clone())
        .collect();
...
mod performance_tests {
    ...
    async fn test_bulk_content_processing() {
        ...
        for i in 1..=content_count {
            ...
            let (_stored_content, version) = content_service
                .create_content_item(
                    content_item,
