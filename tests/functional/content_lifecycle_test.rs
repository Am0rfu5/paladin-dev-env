...
mod functional_tests {
    ...
    use paladin::core::base::service::node_version_service::{
        ChangeType, InMemoryNodeVersionRepository, NodeVersionRepository, VersioningError,
    };
    use paladin::core::platform::container::content::{ContentItem, ContentType, TextContent};
    use paladin::core::platform::manager::content_service::{
        ContentItemService, ContentItemServiceError,
    };
    use std::sync::Arc;
    use uuid::Uuid;
    ...
    fn test_functional_content_item_lifecycle() {
        // Setup
        let repository: Arc<dyn NodeVersionRepository<_> + Send + Sync> =
            Arc::new(InMemoryNodeVersionRepository::new());
        ...
        let (mut content_item, initial_version) = service
            .create_content_item(
                content_item,
        ...
        content_item.set_description(Some(
            "A blog post about functional testing in Rust".to_string(),
        ));
        content_item.set_tags(Some(vec![
            "rust".to_string(),
            "testing".to_string(),
        ...
        let (mut content_item, update_version) = service
            .update_content_item(
                content_item,
        ...
        content_item
            .update_content(ContentType::Text(updated_text_content))
            .unwrap();

        let (content_item, content_update_version) = service
            .update_content_item(
                content_item,
                Some("author@example.com".to_string()),
        ...
        let publish_version = service
            .publish_content_item(&content_item, Some("publisher@example.com".to_string()))
            .unwrap();
        ...
        let version_types: Vec<ChangeType> = history
            .versions
            .iter()
        ...
        assert_ne!(
            comparison.version1.node_data.content,
            comparison.version2.node_data.content
        );

        // Test 8: Restore to previous version
        let (restored_content, restore_version) = service
            .restore_version(content_id, 2, Some("admin@example.com".to_string()))
            .unwrap();
        ...
        assert!(
            restore_version
                .change_summary
                .unwrap()
                .contains("Restored from version 2")
        );
        ...
        let archive_version = service
            .archive_content_item(&restored_content, Some("admin@example.com".to_string()))
            .unwrap();
    ...
    fn test_content_node_integration() {
        ...
        let text_content =
            TextContent::new(None, Some("Testing node integration".to_string())).unwrap();
    ...
    fn test_content_item_basic_functionality() {
        ...
        let text_content =
            TextContent::new(None, Some("Basic functionality test".to_string())).unwrap();

        let mut content_item =
            ContentItem::new_with_title(ContentType::Text(text_content), "Basic Test".to_string())
                .unwrap();
    ...
    fn test_content_type_variations() {
        use paladin::core::platform::container::content::{
            AudioContent, ImageContent, VideoContent,
        };
