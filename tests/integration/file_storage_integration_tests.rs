...
impl FileStorageTestContext {
    ...
    pub fn create_upload_options(&self, tags: Vec<String>) -> UploadOptions {
        ...
        metadata.insert(
            "test_mode".to_string(),
            if self.env.use_external_services {
                "external".to_string()
            } else {
                "local".to_string()
            },
...
mod tests {
    ...
    pub async fn test_file_upload_download_lifecycle() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let (file_path, file_content) =
            ctx.create_test_file("hello.txt", "Hello, integration test!");
        let upload_options =
            ctx.create_upload_options(vec!["test".to_string(), "hello".to_string()]);

        let file_item = ctx
            .adapter
            .upload_file(&file_path, &file_content, Some(upload_options))
            .await?;
    ...
    pub async fn test_file_operations_full_cycle() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let files = vec![
            (
                "analysis/code.rs",
                "fn main() { println!(\"Integration test\"); }",
            ),
            (
                "reports/audit.md",
        ...
        for (path_str, content) in &files {
            ...
            ctx.adapter
                .upload_file(&path, &content_bytes, Some(options))
                .await?;
        }
    ...
    pub async fn test_batch_operations() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let batch_files = vec![
            (
                PathBuf::from("batch/file1.txt"),
                "Content 1".as_bytes().to_vec(),
                Some(ctx.create_upload_options(vec!["batch".to_string()])),
            ),
            (
                PathBuf::from("batch/file2.txt"),
                "Content 2".as_bytes().to_vec(),
                Some(ctx.create_upload_options(vec!["batch".to_string()])),
            ),
            (
                PathBuf::from("batch/file3.txt"),
                "Content 3".as_bytes().to_vec(),
                Some(ctx.create_upload_options(vec!["batch".to_string()])),
            ),
        ];
        ...
        // Test batch download
        let download_paths = batch_files
            .iter()
    ...
    pub async fn test_storage_statistics() -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "✅ Storage statistics: {} files, {} bytes",
    ...
    pub async fn test_error_handling() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let result = ctx
            .adapter
            .upload_file(&invalid_path, b"malicious", None)
            .await;
        assert!(result.is_err());
    ...
    pub async fn test_end_to_end_security_audit_workflow() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        let source_files = vec![
            (
                "audit/src/main.rs",
                "fn main() { println!(\"Security audit\"); }",
                vec!["source", "rust"],
            ),
            (
                "audit/src/lib.rs",
        ...
        for (path_str, content, tags) in &source_files {
            ...
            let file_item = ctx
                .adapter
                .upload_file(&path, content.as_bytes(), Some(options))
                .await?;
        ...
        let report_content = format!(
            ...
            uploaded_files.len(),
            if ctx.env.use_external_services {
                "CI/CD"
            } else {
                "local"
            }
