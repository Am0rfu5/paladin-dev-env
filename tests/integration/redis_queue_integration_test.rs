...
mod queue_integration_tests {
    ...
    use paladin_ports::output::queue_port::{
        BatchQueuePort, PriorityQueuePort, QueueError, QueueManagementPort, QueuePort,
    };
    ...
    async fn test_enqueue_and_dequeue_operations() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(
            length, 1,
            "Expected queue length to be 1 after enqueue, but got {}. Item ID: {}",
            length, item_id
        );

        let dequeued_item = ctx.adapter.dequeue("test-queue").await?;
        assert!(
            dequeued_item.is_some(),
            "Expected to dequeue an item, but got None"
        );
    ...
    async fn test_processing_lifecycle() -> Result<(), Box<dyn std::error::Error>> {
        ...
        ctx.adapter
            .start_processing("test-queue", item_id, worker_id.clone())
            .await?;

        let result_data = json!({"status": "completed", "processed_count": 3});
        ctx.adapter
            .complete_processing("test-queue", item_id, Some(result_data))
            .await?;
    ...
    async fn test_processing_failure_and_retry() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let should_retry = ctx
            .adapter
            .fail_processing("test-queue", item_id, "Simulated failure".to_string())
            .await?;
        assert!(should_retry);
        ...
        let should_retry_again = ctx
            .adapter
            .fail_processing("test-queue", item_id, "Simulated failure again".to_string())
            .await?;
        ...
        ctx.adapter.dequeue("test-queue").await?.unwrap();
        let should_retry_final = ctx
            .adapter
            .fail_processing("test-queue", item_id, "Final failure".to_string())
            .await?;
    ...
    async fn test_batch_operations() -> Result<(), Box<dyn std::error::Error>> {
        ...
        ctx.adapter
            .create_queue("batch-test-queue".to_string(), Some(config))
            .await?;
    ...
    async fn test_priority_queue_operations() -> Result<(), Box<dyn std::error::Error>> {
        ...
        ctx.adapter
            .create_queue("priority-test-queue".to_string(), Some(priority_config))
            .await?;
        ...
        PriorityQueuePort::enqueue_with_priority(
            &*ctx.adapter,
            "priority-test-queue",
            low_item,
            MessagePriority::Low,
        ...
        let first_item = ctx
            .adapter
            .dequeue_highest_priority("priority-test-queue")
            .await?
            .unwrap();
    ...
    async fn test_queue_management_operations() -> Result<(), Box<dyn std::error::Error>> {
        ...
        ctx.adapter
            .create_queue("management-test-queue".to_string(), None)
            .await?;
        ...
        ctx.adapter
            .enqueue("management-test-queue", test_item)
            .await?;

        let dequeued = ctx.adapter.dequeue("management-test-queue").await?;
        assert!(
            dequeued.is_some(),
            "Expected to dequeue an item for management test"
        );

        ctx.adapter
            .start_processing("management-test-queue", item_id, "test-worker".to_string())
            .await?;
        ctx.adapter
            .cancel_item("management-test-queue", item_id)
            .await?;
        ...
        ctx.adapter
            .enqueue("management-test-queue", failed_item)
            .await?;

        let dequeued_failed = ctx.adapter.dequeue("management-test-queue").await?;
        assert!(
            dequeued_failed.is_some(),
            "Expected to dequeue a failed item"
        );

        ctx.adapter
            .start_processing(
                "management-test-queue",
                failed_id,
        ...
        ctx.adapter
            .retry_item("management-test-queue", failed_id)
            .await?;
    ...
    async fn test_simple_failure_debug() -> Result<(), Box<dyn std::error::Error>> {
        ...
        ctx.adapter
            .create_queue("debug-test-queue".to_string(), Some(config))
            .await?;
        ...
        println!(
            "Created item with max_retries: {}",
            test_item.config.max_retries
        );
        ...
        let id1 = item1.id();
        println!(
            "Dequeued item 1 with ID: {}, attempt_count: {}",
            id1, item1.attempt_count
        );

        ctx.adapter
            .start_processing("debug-test-queue", id1, "debug-worker-1".to_string())
            .await?;

        let should_retry_1 = ctx
            .adapter
            .fail_processing("debug-test-queue", id1, "First failure".to_string())
            .await?;
        ...
        if should_retry_1 {
            ...
            let id2 = item2.id();
            println!(
                "Dequeued item 2 with ID: {}, attempt_count: {}",
                id2, item2.attempt_count
            );

            ctx.adapter
                .start_processing("debug-test-queue", id2, "debug-worker-2".to_string())
                .await?;

            let should_retry_2 = ctx
                .adapter
                .fail_processing("debug-test-queue", id2, "Second failure".to_string())
                .await?;
    ...
    async fn test_queue_statistics_and_monitoring() -> Result<(), Box<dyn std::error::Error>> {
        ...
        ctx.adapter
            .create_queue("stats-test-queue".to_string(), Some(config))
            .await?;

        // Enqueue 3 items in the status-test-queue
        let items = (0..3)
            .map(|i| ctx.create_queue_item_for("stats-test-queue", json!({"item": i})))
            .collect::<Vec<_>>();
        ...
        for i in 0..3 {
            if let Some(item) = ctx.adapter.dequeue("stats-test-queue").await? {
                ctx.adapter
                    .start_processing("stats-test-queue", item.id(), format!("worker-{}", i))
                    .await?;
                ctx.adapter
                    .complete_processing("stats-test-queue", item.id(), Some(json!({"result": i})))
                    .await?;
                completed_count += 1;
            }
        }
        ...
        for i in 0..3 {
            ...
            let mut fail_item =
                ctx.create_queue_item_for("stats-test-queue", json!({"fail_test": i}));
            ...
            loop {
                ctx.adapter
                    .start_processing(
                        "stats-test-queue",
                        current_item_id,
                        format!("fail-worker-{}-attempt-{}", i, attempts + 1),
                    )
                    .await?;
                ...
                let should_retry = ctx
                    ...
                    .fail_processing(
                        "stats-test-queue",
                        current_item_id,
                        format!("Intentional failure #{} for item {}", attempts + 1, i),
                    )
                    .await?;
                attempts += 1;
        ...
        assert_eq!(
            stats_after_failures.processing_items, 0,
            "Should have no items in processing"
        );
    ...
    async fn test_item_summaries_and_details() -> Result<(), Box<dyn std::error::Error>> {
        ...
        ctx.adapter
            .create_queue("details-test-queue".to_string(), None)
            .await?;
        ...
        ctx.adapter
            .start_processing("details-test-queue", item_id, "detail-worker".to_string())
            .await?;

        let item_details = ctx
            .adapter
            .get_item_details("details-test-queue", item_id)
            .await?;
        ...
        let summaries = ctx
            .adapter
            .get_item_summaries("details-test-queue", vec![item_id])
            .await?;
    ...
    async fn test_purge_operations() -> Result<(), Box<dyn std::error::Error>> {
        ...
        ctx.adapter
            .create_queue("purge-test-queue".to_string(), None)
            .await?;

        let completed_items = (0..5)
            .map(|i| ctx.create_test_queue_item(json!({"type": "completed", "id": i})))
            .collect::<Vec<_>>();

        let failed_items = (0..3)
            .map(|i| ctx.create_test_queue_item(json!({"type": "failed", "id": i})))
            .collect::<Vec<_>>();
        ...
        for item in completed_items {
            ...
            ctx.adapter
                .start_processing("purge-test-queue", item_id, "purge-worker".to_string())
                .await?;
            ctx.adapter
                .complete_processing(
                    "purge-test-queue",
    ...
    async fn test_error_handling() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let result = ctx
            ...
            .enqueue(
                "non-existent-queue",
                ctx.create_test_queue_item(json!({"test": true})),
            )
            .await;
        ...
        ctx.adapter
            .create_queue("duplicate-test".to_string(), None)
            .await?;
        let result = ctx
            .adapter
            .create_queue("duplicate-test".to_string(), None)
        ...
        let result = ctx
            .adapter
            .get_item_details("duplicate-test", fake_id)
            .await;
    ...
    async fn test_queue_config_updates() -> Result<(), Box<dyn std::error::Error>> {
        ...
        ctx.adapter
            .update_queue_config("config-test-queue", updated_config.clone())
            .await?;
    ...
    async fn test_concurrent_operations() -> Result<(), Box<dyn std::error::Error>> {
        ...
        ctx.adapter
            .create_queue("concurrent-test-queue".to_string(), None)
            .await?;

        let adapter = ctx.adapter.clone();
        let enqueue_handles: Vec<_> = (0..10)
            .map(|i| {
        ...
        let dequeue_handles: Vec<_> = (0..10)
            .map(|_| {
                let adapter = adapter.clone();
                tokio::spawn(async move { adapter.dequeue("concurrent-test-queue").await })
            })
            .collect();
    ...
    async fn test_end_to_end_workflow() -> Result<(), Box<dyn std::error::Error>> {
        ...
        ctx.adapter
            .create_queue("workflow-queue".to_string(), Some(workflow_config))
            .await?;
        ...
        let first_task = ctx
            .adapter
            .dequeue_highest_priority("workflow-queue")
            .await?
            .unwrap();
        ...
        ctx.adapter
            .start_processing("workflow-queue", urgent_id, "urgent-worker".to_string())
            .await?;

        let urgent_result = json!({"status": "completed", "processed_at": "2024-01-01T11:30:00Z", "result": "success"});
        ctx.adapter
            .complete_processing("workflow-queue", urgent_id, Some(urgent_result))
            .await?;
        ...
        for i in 0..5 {
            ...
            let batch_id = task.message.message["data"]["batch_id"]
                .as_str()
                .unwrap()
                .to_string();
            ...
            ctx.adapter
                .start_processing("workflow-queue", task_id, format!("batch-worker-{}", i))
                .await?;
            ...
            if i == 2 {
                ...
                let should_retry = ctx
                    ...
                    .fail_processing(
                        "workflow-queue",
                        task_id,
                        "Network timeout during batch processing".to_string(),
                    )
                    .await?;
                ...
                ctx.adapter
                    .complete_processing("workflow-queue", task_id, Some(result))
                    .await?;
            }
        }
        ...
        let retry_batch_id = retry_task.message.message["data"]["batch_id"]
            .as_str()
            .unwrap();
        assert!(
            processed_batches.contains(&retry_batch_id.to_string()),
        ...
        ctx.adapter
            .start_processing("workflow-queue", retry_id, "retry-worker".to_string())
            .await?;
        let retry_result = json!({"status": "completed", "items_processed": 20, "retry": true});
        ctx.adapter
            .complete_processing("workflow-queue", retry_id, Some(retry_result))
            .await?;
